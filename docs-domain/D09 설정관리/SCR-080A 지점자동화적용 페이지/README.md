...
실물 자산 자동 회수 정책 5종은 docs2 2026-05-23 정합화에 따라 본 화면의 정책 유형 탭(락커·사물함 자동 회수 / 운동복 자동 회수)에서 함께 관리됩니다. (1) **만료 경과 자동 회수** (기본 OFF, Owner(지점장) 조정): ON일 때 매일 02:00에 만료일이 오늘 이전인 장기 락커를 자동 해제하고 회원에게 알림. (2) **이용권 만료 이벤트 자동 회수** (기본 OFF, Owner(지점장) 조정): ON일 때 회원 이용권 종료 이벤트 수신 시 연결 락커를 자동 회수. (3) **일일 사물함 22:00 상태 갱신** (ON 고정, Owner(지점장)도 OFF 불가): 매일 22:00에 만료된 일일 사물함을 overtime 상태로 표시. (4) **일일 사물함 22:00 실제 일괄 회수** (기본 OFF, Owner(지점장) 조정): ON일 때 22:00 상태 갱신 후 overtime 사물함을 실제 일괄 회수하고 알림 발송. (5) **운동복 일일 자동 만료/강제 회수** (기본 OFF, Owner(지점장) 조정): ON일 때 매일 22:00에 당일 미반납 운동복을 회수 처리하고 회원·운영자에게 알림 발송. 이 5종 정책 토글은 Owner(지점장) 외에는 disabled로 표시되며, Owner(지점장) 변경 시 DLG-080A-001 정책 적용 확인을 거쳐 저장되고 D06 락커/사물함/운동복 화면에 즉시 반영됩니다. 5종 정책의 ON/OFF 상태는 D06 SCR-050·SCR-051·SCR-055 페이지 상단에 "자동 회수 정책 상태 배지"로 읽기 전용 표시됩니다.
