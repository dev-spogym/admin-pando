...
부모 화면 SCR-087의 행 [삭제] 아이콘이 트리거입니다. owner와 superAdmin/primary만 호출할 수 있으며, owner는 자기 지점 커스텀 역할만 삭제할 수 있습니다.
...
owner는 자기 지점 커스텀 역할만 삭제 가능합니다(RLS branchId 필터). primary·superAdmin은 전 지점 커스텀 역할을 삭제할 수 있습니다. 매니저 이하는 부모 화면 접근 자체가 차단되어 이 모달을 볼 수 없습니다.
...
owner는 자기 지점 커스텀 역할만 삭제 가능합니다(RLS branchId 필터).
