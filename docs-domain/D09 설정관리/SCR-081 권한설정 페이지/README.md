...
역할 카드 목록의 정렬은 기본 템플릿 8종(superAdmin → primary → owner → manager → fc → trainer → staff → readonly)이 먼저 표시되고 그 아래에 커스텀 역할이 생성 순서대로 따라오는 고정 정렬이 적용됩니다. 운영자가 정렬 순서를 변경할 수는 없습니다.
...
권한 템플릿 8종(superAdmin·primary·owner·manager·fc·trainer·staff·readonly)은 시스템 표준 역할입니다. `PT트레이너`, `GX강사`, `골프강사`는 직원 직무값이며 기본 권한 템플릿은 `trainer`로 지정합니다. 카운터/현장 지원 담당자는 별도 역할을 만들지 않고 `staff` 권한 템플릿으로 통일합니다.
