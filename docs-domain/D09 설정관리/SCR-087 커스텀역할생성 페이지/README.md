...
Owner(지점장)은 RLS branchId 필터로 자기 지점 역할만 노출됩니다.
...
예외 케이스: 이름 빈값·30자 초과·기본 역할명 중복·지점 내 이름 중복·복사 베이스 변경 시 편집 폐기·삭제 시 배정 직원 1명 이상(DLG-087-001 강제)·동시 편집 충돌(30초 lock)·Owner(지점장) 타 지점 접근·매니저 이하 접근·저장 네트워크 오류·매트릭스 로딩 실패·미저장 이탈·본인 권한 회수 경고·생성 후 SCR-081 미노출(새로고침 안내) 등.
...
flowchart TD
    A([커스텀 역할 진입]) --> B{권한}
    B -->|매니저 이하| C[차단]
    B -->|Owner(지점장) / primary / superAdmin| D[역할 목록 fetch]
    D --> E{사용자 액션}
    E -->|새 역할| F[폼 모드 진입]
    E -->|수정| G[폼 모드 채워진 상태]
    E -->|삭제| H{배정 직원}
...
커스텀 역할 이름은 1~30자 필수이며 빈값과 30자 초과는 차단됩니다. 기본 7종 역할명(superAdmin/primary/Owner(지점장)/manager/fc/staff/readonly) 및 한글 표기와 중복은 차단됩니다.
...
Owner(지점장)은 소속 지점 커스텀 역할만 생성·수정·삭제 가능합니다(RLS branchId 필터).
