...
부모 화면 SCR-081의 좌측 패널 [+ 새 역할] 버튼이 트리거입니다. owner·primary·superAdmin만 호출할 수 있으며, owner는 자기 지점 역할만 생성 가능합니다.
...
역할 이름은 1~30자 필수입니다. 기본 7종 역할명(superAdmin/primary/owner/manager/fc/staff/readonly) 및 한글 표기와 중복 차단이 적용되며, 동일 지점 내 커스텀 역할명 중복도 차단됩니다. 다른 지점은 같은 이름을 허용합니다. 인라인 검증으로 글자 수와 중복이 실시간 표시됩니다.
