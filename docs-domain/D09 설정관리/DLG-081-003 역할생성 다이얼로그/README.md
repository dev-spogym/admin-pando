...
부모 화면 SCR-081의 좌측 패널 [+ 새 역할] 버튼이 트리거입니다. Owner(지점장)·primary·superAdmin만 호출할 수 있으며, Owner(지점장)은 자기 지점 역할만 생성 가능합니다.
...
Owner(지점장)은 자기 지점 커스텀 역할만 생성 가능합니다(RLS branchId 필터). primary·superAdmin은 전 지점 역할을 생성할 수 있습니다.
...
Owner(지점장)은 자기 지점 역할만 생성 가능합니다(RLS branchId 필터).
