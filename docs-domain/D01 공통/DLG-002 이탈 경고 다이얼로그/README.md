...
superAdmin, primary, Owner(지점장), 매니저(manager), FC(fc), 스태프(staff), 트레이너(trainer), 프런트(front), 읽기 전용(readonly) 모든 역할이 본 다이얼로그를 동일한 모양과 동작으로 만나며, 어떤 역할에서도 본 다이얼로그가 숨겨지거나 비활성화되지 않습니다.
