...
superAdmin, primary, Owner(지점장), 매니저(manager), FC(fc), 스태프(staff), 트레이너(trainer), 읽기 전용(readonly) 모든 역할이 로그인 상태에서도 본 화면에 진입해 본인 비밀번호를 변경할 수 있습니다. 본인 비밀번호 변경은 보안상 모든 역할의 본인 권한에 속하므로 차별이 없습니다.
