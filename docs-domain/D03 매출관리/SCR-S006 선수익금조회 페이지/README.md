...
두 번째 경로는 대시보드(SCR-D001)의 선수익금 카드 클릭입니다. "잔여 선수익금" 카드를 누르면 잔여 큰 계약 정렬로, "이번 달 인식 예정" 카드를 누르면 종료일 임박 정렬로 자동 진입합니다.
...
슈퍼관리자(superAdmin)와 본사 최고관리자(primary)는 기간 필터, 전체 조회, 본사 통합 모드, 엑셀 내보내기, 월별 보고서 다운로드까지 모두 가능합니다.

센터장(owner)과 매니저(manager)는 자기 지점의 기간 필터, 조회, 엑셀 내보내기, 월별 보고서 다운로드가 가능합니다. 본사 통합 모드는 표시되지 않습니다.
