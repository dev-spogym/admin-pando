...
superAdmin와 primary는 전체 지점의 결제 건에 대해 세금계산서 발행이 가능합니다. 본사 통합 모드에서도 동작이 동일합니다.

Owner(지점장)과 매니저(manager)는 자기 지점의 결제 건에 대해 세금계산서 발행이 가능합니다(SAL-EXT-02 "매니저+ 만 발행" 정책).
