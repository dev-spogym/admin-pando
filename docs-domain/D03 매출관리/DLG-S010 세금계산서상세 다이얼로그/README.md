...
슈퍼관리자(superAdmin)와 본사 최고관리자(primary)는 전체 지점의 모든 세금계산서를 조회하고 이메일 재발송·재발행·마이너스 발행을 수행할 수 있습니다. 본사 통합 모드에서도 동작이 동일합니다.

센터장(owner)과 매니저(manager)는 자기 지점의 세금계산서를 조회하고 이메일 재발송·재발행까지 수행할 수 있습니다. 마이너스 발행은 슈퍼관리자만 가능하므로 버튼이 노출되지 않습니다.
