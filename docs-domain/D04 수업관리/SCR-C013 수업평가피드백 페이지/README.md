...
| 연계 화면 | SCR-C006 강사 근무, STF-02 직원 상세, SCR-094 KPI, SCR-M004 회원 상세 |
| 호출 다이얼로그 | (없음 — 피드백 상세는 행 클릭 드로어, 신고/숨김/삭제는 인라인 처리) |

## 2. 화면 개요와 목적

수업 평가/피드백 페이지는 수업 완료 후 회원이 회원앱에서 남긴 평점·후기를 운영자가 조회하고 강사별 NPS와 응답률을 한 화면에서 확인하는 화면입니다. Owner(지점장)은 월간 강사 평가 자료를, 매니저는 낮은 평점 강사 코칭 자료를 본 화면에서 추출하고, 부적절 후기는 신고 처리로 숨김·삭제 처리합니다. 강사 NPS는 본 화면 데이터로부터 자동 집계되어 STF-02 직원 상세에 반영됩니다.

낮은 평점(1~2점) 등록 시 Owner(지점장)+에게 즉시 알림이 발송되어 빠른 회원 응대를 돕고, 응답률 < 30%일 때는 마케팅 캠페인을 발송할 수 있습니다.

## 3. 진입 경로와 인접 화면

사이드바의 "수업관리 > 수업 평가/피드백"으로 진입합니다. 인접 화면은 SCR-C006 강사 근무, STF-02 직원 상세(NPS), SCR-094 KPI 대시보드, SCR-M004 회원 상세입니다.
...
성공 토스트는 "후기를 숨김 처리했어요", "후기를 삭제했어요"입니다. 신고 N건 자동 숨김 시 "검토 대기" 라벨이 표시됩니다. 낮은 평점(1~2점) 등록 시 Owner(지점장)+에게 즉시 알림이 자동 발송됩니다. 회원 자기 후기 신고 시도는 차단되고 안내가 표시됩니다.

## 9. 데이터 흐름과 외부 연동

API는 피드백 목록 `GET /api/lesson-feedback`, 요약 `GET /api/lesson-feedback/summary`, 신고 `POST /api/lesson-feedback/:id/report`, 숨김 `POST /api/lesson-feedback/:id/hide`입니다. NPS 자동 집계 잡(매일 04:00)이 STF-02 직원 상세 NPS를 갱신합니다. 낮은 평점 즉시 알림 잡은 1~2점 등록 시 Owner(지점장)+에게 NFR-19 알림을 발송합니다. 자동 숨김 잡은 신고 N건(예: 3건) 이상 시 자동 숨김 + 매니저 검토 알림을 발송합니다.
...
superAdmin·primary·Owner(지점장)은 신고 처리·삭제(영구)가 가능합니다. 매니저는 신고 처리·숨김이 가능합니다. 트레이너는 본인 수업 피드백만 조회 가능(자기 평가 보호)합니다. FC·스태프는 접근 불가입니다. readonly는 접근 불가입니다.
...
자동 숨김 정책은 신고 N건(예: 3건) 이상 시 자동 숨김 + 매니저 검토 대기이고, 낮은 평점 자동 알림은 1~2점 등록 시 Owner(지점장)+에게 즉시 알림 발송입니다. 응답률 향상 캠페인은 응답률 < 30% 시 마케팅 발송 옵션으로 활성화됩니다.
