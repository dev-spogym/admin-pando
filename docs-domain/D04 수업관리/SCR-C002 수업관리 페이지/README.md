...
슈퍼관리자·본사 최고관리자·Owner(지점장)·매니저는 등록·수정·취소·출석·서명 검토를 모두 수행합니다. 트레이너는 본인 담당 수업만 수정·출석 처리가 가능하고 타 강사 수업은 조회만 됩니다. FC·스태프는 전체 수업 목록 조회만 가능하고 등록·수정 버튼이 보이지 않습니다. readonly는 화면 자체에 접근할 수 없습니다. 본사 통합 모드에서 지점 컬럼은 슈퍼관리자·primary에게만 노출됩니다.
