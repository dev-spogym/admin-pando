...
강사 근무 현황 페이지는 강사별 수업 배정 건수·근무 시간·OT 운영 현황(배정·1차·2차·거절·이월)을 한 화면에서 요약 비교하는 인사·KPI 자료 화면입니다. 센터장은 월간 인사 평가, 매니저는 OT 배정 재조정, 본사 슈퍼관리자는 트레이너별 OT 배정 합계와 SCR-094 KPI의 1:1 매칭 검증, 트레이너는 본인 근무 기록 확인, 급여 정산(SCR-064) 자료 다운로드까지 본 화면 데이터로 처리됩니다.
