...
예외로 출석완료/노쇼 행 일반 취소 시도는 "상태 정정으로만 변경할 수 있어요" 차단, 취소 사유 미입력은 인라인 에러, 트레이너의 타 강사 처리 시도는 행 read-only, 노쇼 → 출석 정정 시 자동 페널티 해제, 만료 또는 잔여 0 회원 출석 처리 시도는 출석은 처리 + 차감 실패 토스트 + 운영자 조정 안내, 결제 미완 수업 취소는 SCR-S007 환불 안내, 중복 예약 의심은 빨강 배지 + 정정 권장, 권한 부족은 액션 숨김, 엑셀 10,000행 초과는 기간 축소 안내, 동시 처리 충돌은 낙관적 락 재로드, 회원 탈퇴 후 미처리 예약은 자동 취소, 본사 통합 권한 부족은 지점 컬럼 숨김이 적용됩니다.
