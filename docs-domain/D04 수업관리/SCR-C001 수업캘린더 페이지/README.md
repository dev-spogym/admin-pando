...
superAdmin와 primary는 모든 기능을 사용합니다. 헤더에 지점 전환 드롭다운이 노출되어 전체 지점(통합)과 개별 지점을 자유롭게 전환할 수 있고, 통합 모드에서는 모든 지점의 일정이 합산 조회되어 블록과 카드에 지점 라벨이 함께 표시됩니다. 일괄 변경·일괄 승인·일괄 취소·강사 이관·본사 예외 정책 적용·외부 캘린더 동기화 관리까지 모두 가능합니다.
