...
슈퍼관리자(superAdmin)와 본사 최고관리자(primary)는 전체 지점 로그를 조회할 수 있고 필터·내보내기 모두 가능합니다. RLS 우회 권한을 가지며 모든 감사 로그에 접근 가능합니다.

센터장(owner)은 소속 지점 로그만 조회 가능하며 필터·내보내기 가능합니다. URL 파라미터로 타 지점을 지정해도 서버 권한 검증으로 자기 지점 데이터만 반환됩니다.
