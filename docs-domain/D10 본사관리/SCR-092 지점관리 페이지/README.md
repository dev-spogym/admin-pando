...
행의 [수정] 버튼은 인라인 편집 모드를 켜거나 별도 수정 화면을 엽니다. 슈퍼관리자는 전 항목 수정 가능하고, Owner(지점장)은 연락처·주소·운영 시간 같은 일부 항목만 수정 가능합니다. 운영 상태·코드·Owner(지점장) 매핑은 슈퍼관리자 전용입니다.
...
화면 진입 시점에는 지점 목록 API가 호출됩니다. 응답에는 등록된 모든 지점의 기본 정보, 회원 수(활성), 직원 수, 운영 상태가 포함됩니다. 본사 슈퍼관리자는 RLS 우회로 모든 지점을 보지만, Owner(지점장)은 자기 지점만 반환됩니다.
...
지점 수정 API(`PUT /api/branches/:id`)는 권한 기반으로 수정 가능한 필드가 제한됩니다. 슈퍼관리자는 전 필드, Owner(지점장)은 연락처·주소·운영 시간 같은 제한된 필드만 수정 가능합니다. 운영 상태·코드·Owner(지점장) 매핑 변경은 슈퍼관리자 전용입니다. 모든 변경은 감사 로그에 전·후 값과 함께 기록됩니다.
...
폐점 API(`POST /api/branches/:id/close`)는 활성 회원에 대한 환불·이관이 완료된 후에만 호출 가능하며, MBR-ADV-02 회원 이관 트리거가 연계됩니다. 폐점 후 보관 기간 N개월은 docs2 `_정책미확정_확인필요.md`의 폐점 데이터 보관 기간 항목으로 고객사가 N개월을 확정해야 하는 미확정 정책이며, 확정 전까지는 폐점 상태 전환과 활성 회원 자동 환불·이관 안내까지만 개발 기준으로 삼고 보관 기간 만료 후 자동 아카이브 실행(매일 03:00 배치)은 개발 대기 항목으로 둡니다. 확정 후에는 폐점 후 N개월이 경과하면 자동 아카이브 배치가 데이터를 별도 스토리지로 이관하고 운영 DB에서 삭제합니다.
...
통합 비교 탭의 데이터 격리도 명확합니다. Owner(지점장)은 자기 지점만 비교 가능(사실상 단독 표시), 슈퍼관리자는 RLS 우회로 전 지점 비교 가능합니다.
