...
슈퍼관리자(superAdmin)와 본사 최고관리자(primary)는 모든 지점을 보고 모든 액션을 수행할 수 있습니다. 신규 등록(DLG-092-001), 정보 수정(전 항목), 운영 상태 변경(비활성화/폐점/복원), 통합 비교, Owner(지점장) 매핑, 정책 적용, 아카이브 복원까지 모두 가능합니다. RLS 우회 권한을 가지므로 모든 지점 데이터가 보입니다.
