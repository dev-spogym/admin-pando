...
장비 목록 테이블의 컬럼은 장비명 / 장비 유형 / 설치 위치 / 마지막 점검일 / 다음 점검 예정일 / 점검 주기(일 단위, 퀵 선택 30·90·180·365일 / 즉 월·분기·반기·연간 표기와 등가) / 현재 상태(정상/점검 예정/수리중/고장) / 액션 버튼([점검 등록] · [수리 접수] · [수정] · [삭제])입니다.
...
장비 유형 필터(전체 / 유산소기구 / 웨이트기구 / GX장비 / 기타)는 별도의 드롭다운으로 제공됩니다. 라벨은 DLG-056-001 장비 등록 다이얼로그의 유형 선택과 동일합니다.
...
[장비 등록] 버튼으로 열립니다. 본문에 장비명(필수) · 장비 유형(유산소기구/웨이트기구/GX장비/기타) · 설치 위치 · 점검 주기(일 단위 입력, 퀵 선택 30/90/180/365일) · 첫 점검 예정일 · 메모(선택) 입력 필드가 있습니다.
...
슈퍼관리자 · 본사 최고관리자 · Owner(지점장)은 전체 장비 + 이력을 조회하고 등록 · 수정 · 삭제 · 점검 · 수리 모든 기능을 사용할 수 있습니다.
...
권한별 가시성은 다음과 같이 분리됩니다. 슈퍼관리자 · Owner(지점장)은 등록 · 수정 · 삭제 · 점검 · 수리 모두 가능. 매니저는 점검 · 수리 가능, 삭제 불가. FC · 스태프는 점검 완료 기록만 가능. 트레이너 · 읽기 전용은 조회만 가능합니다.
