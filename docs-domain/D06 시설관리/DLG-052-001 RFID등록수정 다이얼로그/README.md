...
이미 카드 보유 회원 안내 카드의 [기존 분실 후 재발급] 버튼은 기존 카드를 먼저 분실 처리하고 신규 등록을 진행하는 트랜잭션입니다. [추가 허용(권한자만)] 버튼은 슈퍼관리자·Owner(지점장)만 활성화되며 기존 카드 유지 + 추가 발급을 진행합니다.
...
추가 허용(이미 카드 보유 회원에 추가 발급)은 슈퍼관리자 · Owner(지점장)만 가능합니다.
...
회원 1명당 활성 카드 보유 수 정책은 1개가 기본이며 테넌트 토글로 무제한 운영 가능합니다. 정책 위반 시 모달에서 [기존 분실 후 재발급] / [추가 허용 권한자만] / [취소] 선택지가 제공됩니다. 추가 허용은 슈퍼관리자 · Owner(지점장)에 한정됩니다.
...
권한별 가시성: 슈퍼관리자 · Owner(지점장) · 매니저는 모든 작업 가능. FC · 스태프 · 프런트는 등록 가능, 삭제 불가. 트레이너 · 읽기 전용은 조회만 가능합니다.
