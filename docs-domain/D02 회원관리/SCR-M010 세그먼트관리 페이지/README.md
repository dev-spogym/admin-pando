...
자동 분류 세그먼트 7종(신규 / 만료후미등록 / 이탈위험 / 만료임박 / 관심필요 / 충성 / 활발)은 우선순위 기반으로 회원당 단일 라벨이 부여되며 회원 목록(SCR-M001) 배지에 자동 표시됩니다. 운영자 정의 커스텀 세그먼트는 추가 라벨 형태로 제공됩니다.
...
자동 7종 갱신 배치(매일 04:00)가 모든 회원의 단일 라벨을 우선순위 기반으로 갱신합니다. 우선순위는 이탈위험 > 만료임박 > 신규 > 충성 > 활발 > 관심필요 > 만료후미등록 순서이며, 회원이 여러 조건에 해당하더라도 가장 높은 우선순위 라벨 하나만 부여됩니다.
...
자동 7종 우선순위 정책은 회원당 단일 라벨 배정 + 우선순위 순(이탈위험 > 만료임박 > 신규 > 충성 > 활발 > 관심필요 > 만료후미등록)입니다. 회원이 여러 조건에 해당해도 가장 높은 우선순위 라벨 하나만 부여되어 운영자가 우선 액션을 판단하기 쉽게 합니다.
