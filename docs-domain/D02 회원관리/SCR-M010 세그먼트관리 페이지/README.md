...
운영 현장의 시나리오는 다양합니다. 매니저가 월말 이탈 방지 캠페인으로 "30일 미방문 + 활성 + 만료 D-30 이내" 세그먼트를 만들어 카카오 알림톡을 일괄 발송하고, FC가 일일 콜 리스트로 "오늘 만료" + "충성" 자동 세그먼트를 결합해 우선 콜 대상을 확보합니다. 본사 슈퍼관리자는 분기 신규 첫 방문 유도 캠페인으로 "이번 분기 신규 + 첫 방문 X" 세그먼트에 첫 방문 쿠폰을 일괄 발급하고, Owner(지점장)은 "활발" 세그먼트 회원 수 추이로 지점 KPI를 정량 측정합니다.
...
superAdmin와 primary는 전체 브랜드 세그먼트를 조회·생성·수정·삭제·액션 실행할 수 있습니다.

Owner(지점장)은 소속 지점 세그먼트를 조회·생성·수정·삭제·액션 실행할 수 있습니다.
...
커스텀 세그먼트 권한 정책은 본사 슈퍼관리자·Owner(지점장)만 변경 가능, 매니저는 생성·수정만, 삭제는 Owner(지점장)+, FC는 메시지 발송만입니다.
