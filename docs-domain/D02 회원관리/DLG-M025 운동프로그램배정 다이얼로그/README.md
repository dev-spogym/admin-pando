...
이 다이얼로그는 회원 상세 페이지(SCR-M004)의 운동 프로그램 탭에서 "프로그램 배정" 버튼을 누를 때 열립니다. 운동 프로그램 탭은 확장 탭 영역에 위치하며, 트레이너·매니저·Owner(지점장)·슈퍼관리자에게만 보입니다.
...
담당 트레이너는 필수이며, 자기 지점 활성 트레이너 목록에서 선택해야 합니다. 기본값은 현재 로그인한 트레이너이며, 매니저·Owner(지점장)이 다이얼로그를 호출하는 경우에는 명시적으로 트레이너를 지정해야 합니다.
...
슈퍼관리자(superAdmin), 본사 최고관리자(primary), Owner(지점장), 매니저(manager), 트레이너(trainer)는 프로그램 배정 권한이 있습니다.

트레이너는 본인이 담당하는 회원에 대해서만 배정 가능하며, 다른 트레이너 담당 회원의 운동 프로그램 탭에서는 배정 버튼이 보이지 않거나 클릭 시 권한 안내가 표시됩니다.

매니저·Owner(지점장)은 자기 지점 모든 회원에게 배정 가능하지만, 일반적으로는 담당 트레이너가 배정하는 것이 운영 가이드입니다. 트레이너 부재나 인수인계 상황에서 매니저·Owner(지점장)이 대신 처리할 수 있습니다.
...
담당 트레이너 우선 정책은 회원 운동 책임 구조를 분명히 합니다. 매니저·Owner(지점장)이 배정 권한을 가지지만 일반적으로는 담당 트레이너가 배정하는 것이 가이드이며, 트레이너 부재 시에만 매니저가 대신 처리합니다. 이는 회원의 운동 계획이 한 명의 트레이너 책임 하에 일관되게 운영되어야 한다는 운영 철학의 결과입니다.
