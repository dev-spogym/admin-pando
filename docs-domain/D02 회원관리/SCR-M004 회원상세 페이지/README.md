...
결제이력 탭(MBR-02-07)은 결제 건의 간략 목록을 시간 역순으로 표시합니다. 행 클릭 시 DLG-M014 결제 상세 팝업이 열려 결제 수단·이용권·할인·결제경로·결제지점·매출 귀속 지점·정산 지점을 자세히 볼 수 있고, 권한자에게는 환불 버튼(DLG-M013)이 표시됩니다. 결제링크가 만료되었거나 취소된 건에는 권한자(슈퍼관리자·Owner(지점장)·매니저·FC)에게 "재발송" 버튼이 노출되며, 발송은 일 3회로 제한됩니다.
...
결제링크 재발송 정책은 권한자(슈퍼관리자·Owner(지점장)·매니저·FC)만 가능하며 발송 횟수 일 3회 한도입니다. 무분별한 재발송으로 회원이 피로감을 느끼는 것을 막기 위한 정책입니다.
