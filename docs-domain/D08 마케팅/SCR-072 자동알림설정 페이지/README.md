...
superAdmin·primary: 본사 step 정의 + 전 지점 알림 정책 + 마스터 토글 + 신규 규칙 추가.

Owner(지점장)·매니저: 소속 지점 알림 활성/비활성 + 상세 설정 + 신규 규칙 추가 + 마스터 토글. 본사 step 비활성 불가.
...
발송 한도 정책: 동일 회원에게 발송되는 영업/광고성 자동 알림은 SCR-071 수동 발송과 합산해 하루 5회로 제한합니다. 5회 한도 도달 시점 이후의 광고성 자동 알림 트리거는 자동으로 건너뛰어지며 그 사유가 SCR-072A 운영 현황의 "변수/한도 누락" 카운트로 누적 표시됩니다. 정보성 알림(결제·환불·홀딩 종료·이용권 만료 D-Day)은 한도 산정에서 제외됩니다. 일배치(매일 04:00) 광고성 자동 알림이 동일 지점에서 50건 이상 일괄 발송되는 경우 본사 슈퍼관리자(`superAdmin`)에게 알림 센터로 사전 알림이 발송되어 정책 우회를 감지합니다.
