...
superAdmin·primary: 전 지점 통합 데이터 + 통합 모드. 본사 정책 효과 분석 가능.

Owner(지점장)·매니저: 소속 지점 데이터 + 정책 편집(SCR-072) 이동 가능 + FC 후속 액션 큐 전체 관리.
...
권한별 데이터 범위: 슈퍼관리자는 전 지점 통합, 매니저/Owner(지점장)은 소속 지점, FC는 본인 담당. 트레이너·스태프·readonly 차단.
