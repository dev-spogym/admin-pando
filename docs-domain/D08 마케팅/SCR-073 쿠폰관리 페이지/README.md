...
행 [삭제] 버튼은 DLG-073-003을 엽니다. 슈퍼관리자/센터장만 hard delete 가능. 매니저는 정책에 따라 가능 또는 불가. FC는 hidden.
...
권한 차이: 슈퍼관리자·센터장 hard delete 가능. 매니저는 정책에 따라. FC는 [삭제] 버튼 hidden.
...
슈퍼관리자·본사 최고관리자: 전 지점 쿠폰 + 본사 통합 쿠폰 등록 + 생성·수정·삭제·발급 모두.

센터장·매니저: 소속 지점 쿠폰 + 생성·수정·발급. 삭제는 정책에 따라(센터장 가능, 매니저는 옵션).
...
권한별 가시성: 슈퍼관리자 전 지점 + 본사 통합. 센터장 소속 지점 + 생성·수정·삭제·발급. 매니저 생성·수정·발급(삭제는 정책에 따라). FC 조회 + 발급. 트레이너·스태프·readonly 차단.
