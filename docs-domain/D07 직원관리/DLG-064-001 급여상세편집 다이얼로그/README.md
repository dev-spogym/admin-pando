...
본사 슈퍼관리자(superAdmin) · 본사 최고관리자(primary) · 센터장(owner) · 매니저(manager)는 이 모달을 호출하고 저장할 수 있습니다. 매니저+ 권한이 모두 동일한 UX를 경험합니다. 본사 권한자는 본사 통합 모드에서 다른 지점 직원 급여도 편집할 수 있으며, 센터장·매니저는 자기 지점 직원 급여만 편집합니다.
...
확정 권한은 별도이며, 매니저는 편집은 가능하지만 [확정] 자체는 센터장+만 가능합니다. 이는 권한 분리 정책의 핵심이며, 매니저가 편집 후 센터장이 검토·확정하는 워크플로우가 자연스럽게 형성됩니다.
...
확정 상태 검증 정책은 확정된 행에서 이 모달이 read-only로 표시되도록 합니다. 편집하려면 먼저 SCR-064에서 [확정 취소]를 거쳐야 하며, 확정 취소는 센터장+ 권한자만 가능합니다. 이는 확정된 급여의 무결성을 보장하는 정책입니다.
...
권한 분리 정책에 따라 매니저는 편집은 가능하지만 [확정] 자체는 센터장+만 가능합니다. 매니저가 편집 후 센터장이 검토·확정하는 워크플로우가 자연스럽게 형성되며, 잘못된 확정으로 인한 사고를 방지합니다.
