...
본사 슈퍼관리자·본사 최고관리자·owner(센터장)·manager(매니저)는 SCR-061에서 직원 등록·수정이 가능하므로 이 다이얼로그를 호출하게 됩니다. 모든 권한이 동일한 UX를 경험합니다.

trainer·fc·staff·front(트레이너·FC·스태프·프런트)는 SCR-061에 진입할 수 없으므로 이 다이얼로그를 만날 수 없습니다.

readonly는 SCR-061 자체에 접근할 수 없으므로 이 다이얼로그를 만날 수 없습니다.
