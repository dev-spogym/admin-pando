...
본사 슈퍼관리자(superAdmin) · 본사 최고관리자(primary) · Owner(지점장) · 매니저(manager)는 SCR-061에서 직원 등록·수정이 가능하므로 이 다이얼로그를 호출하게 됩니다. 모든 권한이 동일한 UX를 경험합니다. 본사 권한자는 본사 통합 모드에서 다른 지점 직원도 등록·수정할 수 있으며, Owner(지점장)·매니저는 자기 지점 직원만 등록·수정합니다.
