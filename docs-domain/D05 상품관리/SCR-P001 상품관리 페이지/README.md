...
| 호출 다이얼로그 | DLG-P001 상품등록모달, DLG-P001 전지점배포, DLG-P002 상품수정모달, DLG-P002 작업취소확인, DLG-P003 가격이력, DLG-P003 상품삭제확인, DLG-P004 비활성화안내, DLG-P005 상품가져오기, DLG-P006 상품삭제확인, DLG-P007 상품이미지업로드, DLG-P008 가격이력조회 |
...
통계 카드 아래에는 3단계 필터 영역이 있습니다. 1단계는 상품 유형 탭으로 전체·회원권·수강권·대여권·일반의 다섯 가지가 가로로 나란히 배치됩니다. 2단계는 상품 대분류 드롭다운으로 전체·이용권·PT·GX·상품·서비스 중에서 선택하며, 이 값은 내부적으로 `category` 컬럼의 MEMBERSHIP·PT·GX·PRODUCT·SERVICE에 매핑됩니다. 3단계는 종목과 세부종목 필터로 2단계 선택값에 따라 동적으로 표시되는데, 1단계에서 MEMBERSHIP을 고르면 헬스·골프·수영·복싱·크로스핏·기타가, PT를 고르면 PT·재활PT·그룹PT·기타가, GX를 고르면 요가·필라테스·스피닝·줌바·에어로빅·GX 기타의 6종이 고정으로 노출됩니다. PRODUCT는 운동복·락커·식음료·용품·기타, SERVICE는 부가서비스·대여·기타입니다. 우측 상단에는 상품명 검색 입력창이 있고 300ms 디바운스가 적용되어 타이핑이 끝나면 자동으로 결과가 갱신됩니다.
...
이 화면은 두 단계의 탭으로 구성됩니다. 1차는 상품 목록 / 분류 관리의 보기 전환 탭이고, 2차는 상품 목록 보기 안에서의 상품 유형 탭(전체·회원권·수강권·대여권·일반)입니다.
...
2차 탭(상품 유형 탭)은 상품 목록 보기 안에서 작동합니다. 전체 탭은 모든 상품을, 회원권 탭은 category=MEMBERSHIP, 수강권 탭은 PT 또는 GX, 대여권 탭은 PRODUCT 중에서 productType=RENTAL, 일반 탭은 PRODUCT 중에서 GENERAL 또는 SERVICE를 표시합니다. 이 탭을 바꿔도 검색어와 다른 필터는 유지되고, 현재 페이지만 1페이지로 초기화됩니다. 2차 탭 우측에 작은 카운트 배지가 표시되어 운영자가 탭을 누르지 않고도 유형별 분포를 즉시 파악할 수 있습니다.
...
"전 지점 배포" 버튼은 슈퍼관리자에게만 보이며 DLG-P001 전지점배포 모달을 호출합니다. 모달에서 배포할 상품과 대상 지점을 선택한 뒤 일괄 배포를 실행하고, 배포 결과는 진행률 모달 + 성공/실패 지점 분리 표시로 보고됩니다.
...
상세 패널 내부에서 호출되는 모달들(DLG-P003 가격이력, DLG-P005 상품가져오기, DLG-P007 상품이미지업로드, DLG-P008 가격이력조회)은 상세 패널의 책임 범위이므로 SCR-P003 문서에서 자기완결로 다시 정의됩니다. 이 화면의 책임은 상세 패널 진입과 모달 호출이 안전하게 일어나는 것까지입니다.
...
센터장(owner)은 자기가 속한 지점의 상품 전체를 조회할 수 있습니다. 본사 통합 모드 토글은 표시되지 않거나 비활성화되며, 자기 지점에 한정해서 작업합니다. 등록·수정·삭제·비활성·이미지·패키지·엑셀 다운로드까지 수행할 수 있고, 본사 모드 전용 기능(전 지점 배포)은 사용할 수 없습니다.
...
flowchart TD
    ...
    Q -->|>1만| S[비동기 배치 모드]
    S --> T[알림 센터 푸시]
    K -->|전 지점 배포 superAdmin| U[DLG-P001 전지점배포]
    U --> V[진행률 모달]
    V --> W{결과}
...
상품 유형 배지는 색상과 라벨이 동시에 인식되어야 합니다. 이용권은 파랑 계열, PT는 보라 계열, GX는 주황 계열, 상품은 회색 계열, 서비스는 청록 계열로 강하게 구분하여 운영자가 멀리서 봐도 색만으로 상품 유형을 짐작할 수 있어야 합니다. 색맹 운영자를 고려해 텍스트 라벨이 항상 색상과 함께 표시됩니다.
...
상품 분류 체계는 1단계 대분류와 2단계 세부 분류로 구성된 2단계 체계가 정본입니다. 1단계 대분류는 MEMBERSHIP·PT·GX·PRODUCT·SERVICE의 5종이며, 2단계 세부종목은 대분류에 따라 다른 옵션을 가집니다. GX 대분류는 시스템 고정 6종(요가·필라테스·스피닝·줌바·에어로빅·GX 기타)이며 이 6종은 상품 등록·상품 목록 필터·매출통계·KPI 대시보드의 GX 세부 종목 분포에서 같은 기준으로 사용됩니다. GX 외의 분류는 분류 관리 탭에서 자유롭게 추가할 수 있지만 GX 6종은 시스템 코드 변경이 필요한 슈퍼관리자 권한이어야 추가가 가능합니다.
...
상품 삭제는 보수적으로 운영됩니다. 판매 이력이 0건이면 완전 삭제가 가능하지만, 1건 이상이면 비활성 전환만 가능합니다. 패키지 구성 상품으로 사용 중이면 패키지 분리 후에만 삭제 가능하며 강제 삭제는 차단됩니다. 삭제 권한은 슈퍼관리자·센터장·상품 편집 권한자에 한정되고, 매니저는 비활성 전환만 가능합니다.
