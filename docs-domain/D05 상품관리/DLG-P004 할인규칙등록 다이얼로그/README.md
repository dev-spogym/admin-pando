...
SCR-P004 할인 설정 페이지의 "할인 추가" 버튼을 클릭하면 호출됩니다. 권한이 있는 슈퍼관리자·센터장·매니저·관리자 권한 보유자에게만 버튼이 보이므로 모달 호출이 가능합니다.
...
슈퍼관리자·센터장·관리자 권한·매니저가 모달 진입과 등록이 가능합니다. 일반 직원과 readonly는 진입 자체가 차단됩니다.

## 9. 메인 흐름

```mermaid
flowchart TD
    A([할인 추가 버튼]) --> B[DLG-P004 등록 모달]
    B --> C[할인명+유형+값+조건+한도]
    C --> D[적용 상품 + 자동 조건]
    D --> E[기간+중복정책+횟수제한]
    E --> F[미리보기 시뮬레이션 선택]
