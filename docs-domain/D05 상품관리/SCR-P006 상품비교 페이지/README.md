...
이 화면에서 빠져나가는 인접 화면으로는 "이 상품 결제하기" 버튼으로 진입하는 결제 화면(SCR-S003), 상품 카드 클릭으로 점프하는 상품 상세 패널(SCR-P003), 회원 검색 모달(SCR-M001)로 점프하는 회원 선택, 그리고 상품 삭제 시 호출되는 DLG-P006 상품삭제확인이 있습니다.
...
삭제 버튼(어드민 + 권한자만)은 DLG-P006 상품삭제확인 모달을 호출하여 그 상품을 시스템에서 완전 삭제하거나 비활성 전환합니다. 비교 화면에서 삭제하면 해당 컬럼이 자동으로 제거되고 "삭제됨" 토스트가 표시됩니다.
...
슈퍼관리자(superAdmin)와 본사 최고관리자(primary)는 비교·삭제·결제 점프·공유 모두 가능합니다.

센터장(owner)도 비교·삭제·결제 점프·공유 모두 가능합니다.
...
상품 삭제 권한 제한 정책은 비교 화면에서 상품 삭제는 슈퍼관리자/센터장/상품 편집 권한자만 가능하며 일반 매니저는 비교·결제 점프·공유만 가능하다는 것입니다.
...
권한별 액션 정책은 슈퍼관리자/센터장은 비교·삭제·결제 점프·공유, 매니저는 비교·결제 점프·공유(삭제 제외), FC는 비교·결제 점프·공유(담당 회원 한정 결제 점프), 일반 직원은 비교 조회, readonly는 조회만입니다.
