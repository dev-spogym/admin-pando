...
SCR-P001 상품 관리 화면에서 헤더 우측의 "전 지점 배포" 버튼을 클릭하면 중앙 모달로 열립니다. 이 버튼은 슈퍼관리자(superAdmin)와 본사 최고관리자(primary)에게만 보이고 다른 권한자에게는 숨겨집니다.
...
헤더 아래에는 상품 선택 영역이 있습니다. 다중 선택 가능한 체크박스 목록으로 본사 표준 상품이 표시되며, 검색창과 카테고리 필터(MEMBERSHIP/PT/GX/PRODUCT/SERVICE)로 좁힐 수 있습니다. 선택된 상품 수가 상단에 "선택된 상품 N개"로 표시됩니다.
...
슈퍼관리자(superAdmin)와 본사 최고관리자(primary)에게만 SCR-P001의 "전 지점 배포" 버튼이 표시되어 모달 진입이 가능합니다. 센터장·매니저·일반 직원·FC·트레이너·readonly에게는 버튼 자체가 숨겨지고 직접 URL 우회 진입도 차단됩니다.
...
전 지점 배포 권한 정책은 슈퍼관리자(superAdmin)와 본사 최고관리자(primary)만 가능하다는 것입니다. 다른 권한자에게는 버튼이 보이지 않고 우회 접근도 차단됩니다.
