...
SCR-P007 재고 관리 화면의 행 액션 영역에서 [조정] 버튼을 클릭하면 호출됩니다. 권한이 있는 슈퍼관리자·Owner(지점장)·매니저·재고 관리 권한자에게만 버튼이 보이므로 모달 호출이 가능합니다.
...
슈퍼관리자·Owner(지점장)·매니저·재고 관리 권한자만 모달 진입과 조정이 가능합니다. 일반 직원과 readonly는 진입 자체가 차단됩니다.
...
권한 정책은 슈퍼관리자·Owner(지점장)·매니저·재고 관리 권한자만 모달 진입이 가능합니다.
