...
이 다이얼로그는 SCR-P007 재고 관리 페이지의 행 액션에서 [입고] 버튼을 눌렀을 때 호출됩니다. 권한은 매니저·Owner(지점장) / manager / 재고 관리 권한을 별도 부여받은 fc·trainer·staff·Owner(지점장)·슈퍼관리자에 한정되며, 일반 직원에게는 [입고] 버튼이 화면에 표시되지 않습니다.
...
슈퍼관리자, 본사 최고관리자, Owner(지점장), 매니저는 입고 등록을 자유롭게 수행할 수 있습니다.

Owner(지점장) / manager / 재고 관리 권한을 별도 부여받은 fc·trainer·staff도 입고 등록이 가능합니다.
...
권한 정책은 매니저·Owner(지점장) / manager / 재고 관리 권한을 별도 부여받은 fc·trainer·staff·Owner(지점장)·슈퍼관리자에 한정됩니다. 일반 직원의 직접 입고를 차단해 재고 변동이 운영자 실수로 잘못 기록되는 것을 막습니다.
