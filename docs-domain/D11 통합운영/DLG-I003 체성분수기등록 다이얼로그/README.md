...
두 번째 경로는 SCR-I007 회원 건강 연동 요약 화면의 체성분 이력 섹션 "측정 데이터 추가" 버튼 클릭입니다. 회원이 결정된 상태로 본 다이얼로그가 열려 회원 검색 단계가 자동으로 건너뛰어집니다.

세 번째 경로는 SCR-M004 회원 상세의 체성분 이력 카드에서 "수기 등록" 액션을 선택한 경우입니다. SCR-I007과 동일하게 회원이 결정된 상태로 진입합니다.

네 번째 경로는 SCR-I006의 오류/검수 탭에서 "수기 등록으로 처리" 액션을 선택한 경우입니다. 이 경우 검수 대기 데이터를 참고 자료로 함께 표시하면서 운영자가 수기로 재입력합니다.
...
owner(센터장)은 자기 지점의 모든 회원에게 체성분 수기 등록이 가능합니다. owner 등록은 정상 값이면 자동 확정 상태로 저장되어 즉시 SCR-I007과 회원 상세에 노출됩니다. 중복 또는 이상치는 검수 대기로 저장됩니다.
