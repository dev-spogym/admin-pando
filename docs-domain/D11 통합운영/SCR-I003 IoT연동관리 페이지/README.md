...
| 작성일 | 2026-05-22 |
| 버전 | v1.1 |
| 상태 | **D11 독립 화면 구현 제외 — IoT 상태·이벤트 조회는 D11 각 화면에 임베드** |
| 도메인 | D11 통합운영 (독립 화면 없음, 조회 전용 임베드) |
| 화면 ID | SCR-I003 |
...
> **이 화면(SCR-I003)은 D11 통합운영에서 독립 운영 화면으로 구현하지 않습니다.**
>
> docs2(D11-통합운영/통합운영.md) 기준: D11은 IoT 장비를 등록·삭제·제어하는 화면을 제공하지 않습니다. D11에서 필요한 것은 통합 출석, 락커 운영, 체성분 화면에 표시할 상태값과 이벤트 수신값뿐입니다.
>
> **D11에서 허용되는 범위:**
...
>
> **D11에서 금지되는 범위:** 기기 추가·삭제, IP/포트 수정, 펌웨어 관리, 원격 게이트 개방, 키오스크 원격 재시작, 로그 다운로드, 자동 전원 스케줄 설정, 출입 규칙·키오스크 화면 구성·TTS·체크인 수단·기기 등록 등 (→ D09-설정관리에서만 처리)
>
> **CRM에서 고정으로 인식하는 기기 종류는 4종입니다: 출입 게이트 / 키오스크 / 락커 컨트롤러 / InBody 측정기.** 카드 리더·얼굴인식 카메라 등은 별도 기기 종류가 아니라 출입 게이트 또는 키오스크의 인증 수단/부속 모듈로 봅니다.
...
superAdmin와 primary는 전 지점 IoT 기기 현황을 조회만 가능합니다. 원격 제어·기기 추가·삭제·규칙·자동 전원 변경은 모두 차단되며, "조회 전용 - 변경은 지점 owner" 안내가 표시됩니다. 펌웨어 분포 점검, 노후 단말 식별, 본사 표준화 권고에 사용됩니다.

Owner(지점장)은 자기 지점의 IoT 기기를 모두 관리할 수 있습니다. 기기 등록·삭제·원격 제어(연결 테스트·재시작·게이트 개방)·출입 규칙·자동 전원 스케줄·전체 동기화까지 모든 액션이 가능합니다. InBody API 토큰 재발급도 가능합니다.
