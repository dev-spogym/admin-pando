...
Owner(지점장)은 자기 지점의 측정 결과 조회·검수·확정·수기 등록·파일 업로드·무시 처리·매칭까지 모든 액션이 가능합니다.
...
수기 등록 권한 정책은 Owner(지점장)은 모든 회원, 매니저는 모든 회원(미확정 상태로 저장 → owner 확정 필요), 강사는 본인 담당 회원만 수기 등록 가능하다는 것입니다. 강사의 수기 등록도 미확정 상태이며 owner 확정이 필요합니다.

검수 처리 권한 정책은 Owner(지점장)만 확정·매칭·무시 처리가 가능하다는 것입니다. 검수 처리는 회원에게 노출되는 데이터의 최종 결정이므로 권한이 단계적으로 제한됩니다.
