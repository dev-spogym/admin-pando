...
| 심각도 | 유형 | 위치 | 참조 | 내용 |
|---|---|---|---|---|
| WARN | `SOURCE_SET_NOT_REFLECTED` | `D03-매출관리\매출관리.md:245` SCR-S004 매출 통계 | `SCR-S004` | 원본 화면에는 `분류` 집합 정의가 있으나 docs4 섹션에서 같은 유형 집합 표현을 찾지 못했습니다: 5종 으로 분류 |
| WARN | `SOURCE_SET_NOT_REFLECTED` | `D03-매출관리\매출관리.md:758` SCR-S009 할부결제 관리 | `SCR-S009` | 원본 화면에는 `탭` 집합 정의가 있으나 docs4 섹션에서 같은 유형 집합 표현을 찾지 못했습니다: 4개 탭 |
...
| WARN | `SET_COUNT_ITEM_MISMATCH` | `D09-설정관리\설정관리.md:1796` DLG-081-006 역할 변경 영향 분석 | `2종 카드` | 표기된 개수는 2개이나 추출된 항목은 4개입니다: 배정 직원 수 / 담당 회원 합계 / 권한 차이 / 재배정 필요 여부 |
