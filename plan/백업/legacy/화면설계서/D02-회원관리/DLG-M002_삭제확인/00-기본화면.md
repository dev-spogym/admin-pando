...
roles:
  ...
  - primary
  - owner
filePath: src/components/members/DeleteMemberDialog.tsx
...
functional:
  - id: F-DM002-01
    title: 단일 회원 삭제(상세 화면) + 복수 회원 삭제(목록 BulkActi…
    description: 단일 회원 삭제(상세 화면) + 복수 회원 삭제(목록 BulkActionBar)
  - id: F-DM002-02
    title: '삭제 = deletedAt = now(), status = ''INAC…'
    description: >-
      삭제 = `deletedAt = now(), status = 'INACTIVE'`로 마킹 (실제 레코드 유지, 30일 후 개인정보
      마스킹)
  - id: F-DM002-03
    title: destructive 레벨
    description: 'destructive 레벨: 파란 버튼 대신 **빨간 danger** 버튼'
  - id: F-DM002-04
    title: 돌이킬 수 없음을 사용자에게 강하게 전달
    description: 돌이킬 수 없음을 사용자에게 강하게 전달
---
