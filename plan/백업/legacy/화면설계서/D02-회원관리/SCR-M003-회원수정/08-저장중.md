...
| 이전 상태 | `07-Step2` |
| 다음 상태 | `09-수정완료 (SCR-M004)` |

## 🧩 바이브코딩 프롬프트
```
Next.js 15 App Router + TypeScript + Tailwind + Supabase 'use client' 컴포넌트 작성.
화면: SCR-M003 회원수정 — 상태: 저장중

파일: src/app/members/edit/page.tsx

SCR-M003 회원수정 — 상태: 저장 중\n\nPUT /api/members/{id} 호출 중. 모든 입력 disabled.\n성공 시 /members/detail?id={id} 이동.\n실패 시 toast.error + Step2 유지.
...
### 사용 시점
PUT /api/members/{id} 호출 중. isSubmitting=true. SCR-M002 저장중과 동일.
