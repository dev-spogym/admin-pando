...
functional:
  - id: F-M004-01
    title: '프로필 헤더 + 만료 알림 배너가 Sticky로 고정, 모든 탭 전환…'
    description: '프로필 헤더 + 만료 알림 배너가 Sticky로 고정, 모든 탭 전환 중에도 회원 기본 정보·액션 버튼에 즉시 접근 가능.'
  - id: F-M004-02
    title: 회원 상태(ACTIVE/HOLDING/EXPIRED/SUSPENDED…
    description: >-
      **회원 상태(ACTIVE/HOLDING/EXPIRED/SUSPENDED/INACTIVE)에 따라 UI·가능 액션·배너·배지가 완전히
      달라짐**.
  - id: F-M004-03
    title: 회원의 전 라이프사이클(등록
    description: 회원의 전 라이프사이클(등록→이용→만료/홀딩/정지/탈퇴/이관)을 단일 페이지에서 관리.
  - id: F-M004-04
    title: 멀티테넌트
    description: >-
      멀티테넌트: URL `?id=`로 전달된 memberId가 요청자의 `branchId` 범위 밖이면 서버에서 403 →
      `/forbidden`.
---
