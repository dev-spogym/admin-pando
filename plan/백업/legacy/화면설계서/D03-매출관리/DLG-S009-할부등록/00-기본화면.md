...
functional:
  - id: F-DS009-01
    title: 등록 시 회차별 스케줄(JSON 배열)을 자동 생성하여 install…
    description: 등록 시 회차별 스케줄(JSON 배열)을 자동 생성하여 `installments.schedule` 컬럼에 저장.
  - id: F-DS009-02
    title: 월 납입액을 실시간 계산하여 사용자가 현금흐름을 사전 검증할 수 있게…
    description: 월 납입액을 실시간 계산하여 사용자가 현금흐름을 사전 검증할 수 있게 한다.
  - id: F-DS009-03
    title: 기본 정책
    description: '기본 정책: **무이자(이자율 0)**, 필요 시 확장 가능 필드로 `interest_rate` 유지(Phase 2).'
  - id: F-DS009-04
    title: branch_id 스코프로 멀티테넌트 격리
    description: '`branch_id` 스코프로 멀티테넌트 격리.'
---
