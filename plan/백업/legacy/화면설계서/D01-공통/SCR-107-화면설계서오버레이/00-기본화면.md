...
platforms:
  ...
  - mobile
route: /?spec=1
filePath: src/components/spec-overlay/SpecOverlay.tsx
component: SpecOverlay
diagrams:
  - docs/다이어그램/D01_공통/SCR-107_화면설계서오버레이/F1_진입.md
  - docs/다이어그램/D01_공통/SCR-107_화면설계서오버레이/F2_메인.md
  - docs/다이어그램/D01_공통/SCR-107_화면설계서오버레이/F3_버튼액션.md
  - docs/다이어그램/D01_공통/SCR-107_화면설계서오버레이/F6_상태별.md
  - docs/다이어그램/D01_공통/SCR-107_화면설계서오버레이/F7_권한.md
  - docs/다이어그램/D01_공통/SCR-107_화면설계서오버레이/F8_에러.md
errorCodes:
  - E403001
  - E404001
  - E404002
...
functional:
  - id: F-107-01
    title: 기획자·QA·개발자가 실제 UI와 설계서를 동시에 비교 할 수 있게 한다
    description: 기획자·QA·개발자가 **실제 UI와 설계서를 동시에 비교** 할 수 있게 한다.
  - id: F-107-02
    title: 상태별(01~N) 문서를 빠르게 탐색할 수 있도록 사이드 목차를 제공
    description: 상태별(`01~N`) 문서를 빠르게 탐색할 수 있도록 사이드 목차를 제공.
  - id: F-107-03
    title: 'Mermaid 다이어그램과 스펙 테이블을 바로 읽고, PDF/MD로…'
    description: 'Mermaid 다이어그램과 스펙 테이블을 바로 읽고, PDF/MD로 스냅샷 다운로드 가능.'
  - id: F-107-04
    title: 원본 CRM 페이지 인터랙션을 중단시키지 않음(pointer-even…
    description: 원본 CRM 페이지 인터랙션을 **중단시키지 않음**(pointer-events 분리 옵션).
  - id: F-107-05
    title: 개발 빌드 및 superAdmin 운영 환경에서만 노출
    description: 개발 빌드 및 `superAdmin` 운영 환경에서만 노출.
---
