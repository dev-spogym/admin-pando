...
- **엔티티**: `Sale.status` / `Payment.status`
- **저장 방식**: DB enum
...
|-----------|------|----|--------|------|----------|---------|
| T-PAY-01 | [신규] | PENDING | POS/전자계약 결제 시작 | STAFF 이상 | 결제 레코드 생성, PG 요청 전송 | TC-PAY-01 |
...
| T-PAY-04 | PENDING | INSTALLMENT | 부분 결제 금액 입력 | STAFF 이상 | unpaidAmount 기록, 미수금 알림 | TC-PAY-04 |
...
| T-PAY-12 | INSTALLMENT | APPROVED | 미수금 완납 처리 | STAFF 이상 | unpaidAmount = 0, 완납 알림 | TC-PAY-12 |
