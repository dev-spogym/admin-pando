...
> 지점 내 최상위 권한. 모든 메뉴 접근 가능. isSystem=true (수정 불가).
...
journey
    ...
    section 회원 관리
      회원 목록 조회 /members: 5: primary
      신규 회원 등록 /members/new: 5: primary
      회원 상세 확인 /members/detail: 5: primary
      체성분 데이터 확인 /body-composition: 4: primary
      출석 현황 확인 /attendance: 4: primary

    section 매출 관리
      매출 현황 조회 /sales: 5: primary
      매출 통계 분석 /sales/stats: 5: primary
      POS 결제 처리 /pos: 4: primary
      환불 처리 /refunds: 3: primary
    ...
    section 직원 및 급여
      ...
      직원 등록 /staff/new: 4: primary
      직원 근태 확인 /staff/attendance: 4: primary
      급여 관리 /payroll: 5: primary
      급여 명세서 /payroll/statements: 4: primary

    section 마케팅
      리드 관리 /leads: 4: primary
      메시지 발송 /message: 4: primary
      쿠폰 관리 /message/coupon: 3: primary
      마일리지 설정 /mileage: 3: primary

    section 설정 및 본사 업무
      센터 설정 /settings: 4: primary
      권한 설정 /settings/permissions: 5: primary
...
| 매출 | 전체 | `/sales*`, `/pos*`, `/refunds*` | ● |
| 상품 | 전체 | `/products*`, `/discount-settings` | ● |
| 시설 | 전체 | `/locker*`, `/rfid*`, `/rooms*` | ● |
| 직원 | 전체 | `/staff*` | ● |
| 급여 | 전체 | `/payroll*` | ● |
| 마케팅 | 전체 | `/leads`, `/message*`, `/mileage` | ● |
| 설정 | 전체 | `/settings*` | ● |
| 기타 | 공지/출석 | `/notices`, `/attendance` | ● |
