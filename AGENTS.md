...
Cmd+/ 오버레이는 **고객사·외주 개발사가 퍼블리싱된 화면과 기획 설명을 대조**하는 용도입니다. 따라서 **개발 전용 섹션은 자동으로 숨겨집니다** (원본 파일에는 그대로 유지).

**기본 차단 섹션** (제목 매칭):
- `## 컴포넌트 트리` / `## 데이터 계약` / `## API 호출|엔드포인트|통합|스펙`
- `## 전역 상태` / `## Store` / `## 스토어`
- `## 🧩 바이브코딩 프롬프트` / `## 구현 상세` / `## 기술 스펙`
- `## 타입 정의` / `## TypeScript` / `## Prisma` / `## DB 스키마`

**신규 개발 섹션은 prefix 컨벤션**으로 숨김 처리:
```markdown
## 🛠 개발: <제목>     ← 자동 숨김 (Cmd+/ · 다이어그램 뷰어 양쪽)
## 🎨 디자인 토큰       ← 일반 섹션은 노출
```

필터 구현: `src/lib/stripDevSections.ts` (design-doc API + diagrams API 공용)

### 6.2 내부 문서 완비 원칙

외주가 중도 이탈해도 다른 개발사가 즉시 착수할 수 있도록 **파일 자체에는 개발 섹션까지 완비**합니다. Cmd+/에서만 감춰질 뿐, 파일을 직접 열면 전부 보입니다.
