...
roles:
  ...
  - owner
  - manager
filePath: src/components/members/BulkStatusChangeDialog.tsx
component: BulkStatusChangeDialog
diagrams:
  - docs/다이어그램/D02_회원관리/DLG/DLG-M001_상태변경/M1_생명주기.md
  - docs/다이어그램/D02_회원관리/DLG/DLG-M001_상태변경/M2_필드검증.md
  - docs/다이어그램/D02_회원관리/DLG/DLG-M001_상태변경/M3_결과분기.md
...
functional:
  - id: F-DM001-01
    title: 대상
    description: >-
      대상: `ACTIVE`, `EXPIRED`, `HOLDING`, `INACTIVE`, `WITHDRAWN`, `SUSPENDED` 중
      선택
  - id: F-DM001-02
    title: 복수 회원 운영 효율(만료 회원 일괄 비활성 등) 향상 목적
    description: 복수 회원 운영 효율(만료 회원 일괄 비활성 등) 향상 목적
  - id: F-DM001-03
    title: 서버는 jwt 역할·branchId로 범위 강제
    description: '서버는 jwt 역할·branchId로 범위 강제. 잘못된 상태 전이(예: WITHDRAWN → ACTIVE)는 거부'
  - id: F-DM001-04
    title: 일괄 처리 중 일부 실패 허용(부분 성공 응답)
    description: 일괄 처리 중 일부 실패 허용(부분 성공 응답)
---
