...
platforms:
  ...
  - tablet
route: /branches
filePath: src/app/(admin)/branches/page.tsx
component: BranchManagement
diagrams:
  - docs/다이어그램/10_권한매트릭스/R1_역할화면_매트릭스.md
  - docs/다이어그램/D10_본사관리/SCR-092_지점관리/F1_진입.md
...
functional:
  - id: F-092-01
    title: 지점 운영 상황을 한눈에
    description: '지점 운영 상황을 한눈에: 총 지점 수·총 회원·이번달 매출·활성 지점 4개 KPI'
  - id: F-092-02
    title: 상태별 관리 흐름
    description: '상태별 관리 흐름: `운영중 → 임시휴업/폐점`의 2단계 확인 플로우로 데이터 안전 보장'
  - id: F-092-03
    title: 멀티테넌트
    description: '멀티테넌트: `primary/super`는 전 지점, `owner`는 자기 브랜드 소속 지점만 노출'
---
