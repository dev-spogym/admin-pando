...
roles:
  ...
  - primary
  - owner
filePath: src/components/admin/BranchCreateDialog.tsx
diagrams:
...
functional:
  - id: F-D092-001-01
    title: 9개 필드(지점명/코드/연락처/주소/상세주소/관리자/오픈·마감 시간/…
    description: 9개 필드(지점명/코드/연락처/주소/상세주소/관리자/오픈·마감 시간/수용 인원/메모)를 한 화면에서 수집한다.
  - id: F-D092-001-02
    title: '지점 코드는 [자동생성] 버튼 또는 수동 입력 모두 지원하되 중복 검…'
    description: '지점 코드는 `[자동생성]` 버튼 또는 수동 입력 모두 지원하되 중복 검증을 서버가 최종 수행한다.'
  - id: F-D092-001-03
    title: 저장 성공 시 본사 운영 스태프가 해당 지점으로 즉시 전환해 세팅을…
    description: 저장 성공 시 본사 운영 스태프가 해당 지점으로 즉시 전환해 세팅을 이어갈 수 있도록 목록 상단으로 갱신한다.
---
