...
functional:
  - id: F-D001-01
    title: '로그아웃은 파괴적 액션은 아니지만, 작업 중인 폼/캐시 상태를 리셋하…'
    description: '로그아웃은 파괴적 액션은 아니지만, 작업 중인 폼/캐시 상태를 리셋하므로 한 번 확인을 받는다.'
  - id: F-D001-02
    title: isDirty=true 상태에서는 DLG-002(이탈 경고)와 조합되…
    description: '`isDirty=true` 상태에서는 DLG-002(이탈 경고)와 조합되지 않고, 로그아웃이 우선한다(의도 명시성).'
  - id: F-D001-03
    title: 확인 시 SCR-109 로그아웃 처리로 진입(또는 동일 핸들러 실행)
    description: 확인 시 `SCR-109` 로그아웃 처리로 진입(또는 동일 핸들러 실행).
---
