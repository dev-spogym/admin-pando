...
platforms:
  ...
  - mobile
route: /logout
filePath: src/app/(auth)/logout/page.tsx
component: LogoutPage
diagrams:
  - docs/다이어그램/D01_공통/SCR-109_로그아웃/F1_진입.md
  - docs/다이어그램/D01_공통/SCR-109_로그아웃/F2_메인.md
  - docs/다이어그램/D01_공통/SCR-109_로그아웃/F3_버튼액션.md
  - docs/다이어그램/D01_공통/SCR-109_로그아웃/F6_상태별.md
  - docs/다이어그램/D01_공통/SCR-109_로그아웃/F7_권한.md
...
functional:
  - id: F-109-01
    title: 서버 세션 무효화(POST /auth/logout)와 클라이언트 정리…
    description: 서버 세션 무효화(`POST /auth/logout`)와 클라이언트 정리(Zustand/localStorage)를 순서대로 실행.
  - id: F-109-02
    title: 실패해도 클라이언트 클리어는 best-effort 강제 진행
    description: 실패해도 클라이언트 클리어는 **best-effort 강제 진행** → 보안 우선.
  - id: F-109-03
    title: 최종 /login?logout=true 리다이렉트 + 완료 토스트 표시
    description: 최종 `/login?logout=true` 리다이렉트 + 완료 토스트 표시.
---
...
| DLG-001 마스터 | `docs/화면설계서/D01-공통/DLG-001-로그아웃확인/00-기본화면.md` | 확인 모달 (이 화면의 진입 전 단계) |
| DLG-000 마스터 | `docs/화면설계서/D01-공통/DLG-000-세션만료/00-기본화면.md` | 자동 로그아웃 시 |
