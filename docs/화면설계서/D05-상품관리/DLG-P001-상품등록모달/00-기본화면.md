...
roles:
  ...
  - owner
  - manager
filePath: src/components/product/ProductDetailPanel.tsx
diagrams:
...
functional:
  - id: F-DP001-01
    title: 마스터-디테일 패널의 "신규 모드"를 중첩 모달로 제공(독립 라우트…
    description: 마스터-디테일 패널의 "신규 모드"를 중첩 모달로 제공(독립 라우트 없이 `/products` 오버레이).
  - id: F-DP001-02
    title: 필수 필드 + 선택 옵션 + 요일/시간 + 옵션 체크박스 + 패키지…
    description: 필수 필드 + 선택 옵션 + 요일/시간 + 옵션 체크박스 + 패키지 구성까지 단일 폼에서 완결.
  - id: F-DP001-03
    title: '저장 성공 시 목록 자동 refetch, 실패 시 인라인/토스트 에러…'
    description: '저장 성공 시 목록 자동 refetch, 실패 시 인라인/토스트 에러 표시.'
  - id: F-DP001-04
    title: 입력 중 이탈 시 DLG-P002를 통한 안전한 취소 흐름을 제공
    description: 입력 중 이탈 시 DLG-P002를 통한 안전한 취소 흐름을 제공.
---
