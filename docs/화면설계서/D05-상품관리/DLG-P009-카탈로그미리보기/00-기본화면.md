...
functional:
  - id: F-DP009-01
    title: 카탈로그 공개 전 실제 회원 앱 화면을 시뮬레이션하여 노출 순서/이미…
    description: 카탈로그 공개 전 **실제 회원 앱 화면을 시뮬레이션**하여 노출 순서/이미지/설명을 검수한다.
  - id: F-DP009-02
    title: 공개/비공개 토글과 카탈로그 편집의 결과를 즉시 확인하여 QA 주기를…
    description: 공개/비공개 토글과 카탈로그 편집의 결과를 즉시 확인하여 QA 주기를 줄인다.
  - id: F-DP009-03
    title: 본사 관리자(primary/superAdmin)는 지점별 카탈로그 품…
    description: 본사 관리자(primary/superAdmin)는 지점별 카탈로그 품질을 원격으로 점검할 수 있다.
---
