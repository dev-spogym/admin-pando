...
기본 RBAC 7개 역할 × 67개 라우트 전체 접근 권한. 공통 2.2 메뉴 접근 매트릭스 원본 기준.

> `●` = 전체접근(CRUD), `○` = 조회만, `—` = 접근불가
>
> `trainer` / `front` 같은 직무형 세부 권한은 화면별 F7 권한 문서를 우선한다. 독립 RBAC role로 분리할지 여부는 `docs/admin/운영용_기획문서/09_클라이언트_정책미확정_확인필요.md`에서 고객사 확인 항목으로 관리한다.
