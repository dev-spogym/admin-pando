...
flowchart LR
  ...
  RoleCheck -->|": primary"|FullAccess[전체 접근\n등록/수정/삭제]
  RoleCheck -->|": fc 트레이너"|FCAccess[조회만\n등록/수정/삭제 불가]
  RoleCheck -->|": staff 프론트"|Blocked[접근 차단]
...
| 역할 | 접근 | 등록/수정/삭제 | |------|------|--------------| | primary | O | O | | fc(트레이너) | O | X(조회만) | | staff/readonly | X | X |
