...
| `ACTIVE` | 활성 | 시작일 ≤ 오늘 ≤ 만료일, 잔여횟수 | #4CAF50 (녹색) | 비종료 |
...
| T-MSP-06 | ACTIVE | EXPIRED | 배치 스케줄러 ( < today) | 시스템 | 만료 알림 발송, 회원 상태 재계산 | TC-MSP-06 |
...
| T-MSP-10 | HOLDING | ACTIVE | 홀딩 해제 (수동) 또는 도래 | MANAGER 이상 / 시스템 | 남은 기간 재계산, 해제 알림 발송 | TC-MSP-10 |
