...
export const productCreateSchema = z.object({
  category: z.enum(['membership', 'pt', 'gx', 'locker', 'etc'], {
  ...
  cashPrice: z
    .number({ required_error: '현금가를 입력해주세요.' })
    .int('현금가는 정수여야 합니다.')
    .min(0, '현금가는 0 이상이어야 합니다.'),
  cardPrice: z
    .number({ required_error: '카드가를 입력해주세요.' })
    .int('카드가는 정수여야 합니다.')
    .min(0, '카드가는 0 이상이어야 합니다.'),
  period: z
    .number({ required_error: '기간을 입력해주세요.' })
    .int('기간은 정수여야 합니다.')
    .min(1, '기간은 1일 이상이어야 합니다.'),
  kioskExposure: z.boolean({
    required_error: '키오스크 노출 여부를 선택해주세요.',
  }),
