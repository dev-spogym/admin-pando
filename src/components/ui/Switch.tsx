import React, { useId } from "react";
import { cn } from "@/lib/utils";

export interface SwitchProps {
  /** ON/OFF 상태 */
  checked: boolean;
  /** 상태 변경 핸들러 */
  onChange: (checked: boolean) => void;
  /** 레이블 텍스트 */
  label?: string;
  /** 비활성화 여부 */
  disabled?: boolean;
  /** 스위치 크기 */
  size?: "sm" | "md";
  /** 추가 클래스 */
  className?: string;
  /** 접근성 레이블 */
  "aria-label"?: string;
  /** ID */
  id?: string;
}

const TRACK: Record<NonNullable<SwitchProps["size"]>, string> = {
  sm: "h-5 w-9",
  md: "h-6 w-11",
};

const THUMB: Record<NonNullable<SwitchProps["size"]>, string> = {
  sm: "h-3.5 w-3.5",
  md: "h-4 w-4",
};

const THUMB_ON: Record<NonNullable<SwitchProps["size"]>, string> = {
  sm: "translate-x-4",
  md: "translate-x-5",
};

const THUMB_OFFSET: Record<NonNullable<SwitchProps["size"]>, string> = {
  sm: "left-[3px] top-[3px]",
  md: "left-[4px] top-[4px]",
};

export default function Switch({
  checked,
  onChange,
  label,
  disabled = false,
  size = "md",
  className,
  "aria-label": ariaLabel,
  id: propId,
}: SwitchProps) {
  const generatedId = useId();
  const id = propId ?? generatedId;

  const handleClick = () => {
    if (!disabled) onChange(!checked);
  };

  const handleKeyDown = (e: React.KeyboardEvent) => {
    if (e.key === " " || e.key === "Enter") {
      e.preventDefault();
      if (!disabled) onChange(!checked);
    }
  };

  return (
    <label
      htmlFor={id}
      className={cn(
        "inline-flex items-center gap-sm select-none",
        disabled ? "cursor-not-allowed opacity-50" : "cursor-pointer",
        className
      )}
    >
      <div
        id={id}
        role="switch"
        aria-checked={checked}
        aria-label={!label ? ariaLabel : undefined}
        tabIndex={disabled ? -1 : 0}
        onClick={handleClick}
        onKeyDown={handleKeyDown}
        className={cn(
          "relative flex-shrink-0 rounded-full transition-colors duration-200 outline-none",
          "focus-visible:ring-2 focus-visible:ring-primary/30 focus-visible:ring-offset-1",
          TRACK[size],
          checked ? "bg-primary" : "bg-gray-300"
        )}
      >
        <span
          className={cn(
            "absolute rounded-full bg-white shadow-sm transition-transform duration-200",
            THUMB[size],
            THUMB_OFFSET[size],
            checked ? THUMB_ON[size] : "translate-x-0"
          )}
        />
      </div>

      {label && (
        <span className="text-[13px] text-content">{label}</span>
      )}
    </label>
  );
}
