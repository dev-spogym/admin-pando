...
export default function PublishingOverview({
  ...
  const firstCategoryPath = `${basePath}/${PUBLISHING_CATEGORIES[0].slug}`;

  return (
    <PublishingSurface publicView={publicView}>
      <PageHeader
        title={publicView ? "FitGenie CRM 화면 갤러리" : "퍼블리싱 갤러리"}
        description={publicView
          ? "클라이언트 검토용 공개 갤러리입니다. 로그인 없이 현재 퍼블리싱된 화면을 preview 모드로 확인할 수 있습니다."
          : "실제 운영 라우트를 그대로 preview 모드로 묶은 갤러리입니다. 복제용 목업이 아니라 현재 페이지를 그대로 보여주기 때문에 개발자가 화면 구조와 상태를 확인하기 쉽습니다."
        }
        actions={
          <>
            {!publicView && (
      ...
      <div className="grid gap-lg lg:grid-cols-2 xl:grid-cols-3">
        {PUBLISHING_CATEGORIES.map((category) => {
          ...
          return (
            <Link key={category.slug} href={`${basePath}/${category.slug}`} className="block">
              <Card
                variant="elevated"
                padding="lg"
