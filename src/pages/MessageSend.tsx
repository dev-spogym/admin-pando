...
const CHANNEL_CONFIG = {
  kakao: { label: '알림톡', maxLen: 1000, costPerMsg: 8, unit: '원' },
  sms:   { label: 'SMS/LMS', maxLen: 2000, costPerMsg: 30, unit: '원', smsMax: 90, lmsThreshold: 90 },
  push:  { label: '앱 푸시', maxLen: 500, costPerMsg: 0, unit: '원' },
};
...
function RecipientSearchModal({
  ...
  const [tempSelected, setTempSelected] = useState<Recipient[]>(selected);

  const filtered = useMemo(() => {
    if (!query) return MOCK_MEMBERS;
    const q = query.toLowerCase();
    return MOCK_MEMBERS.filter(
      m => m.name.includes(q) || m.phone.includes(q)
    );
  }, [query]);

  const isAllSelected = filtered.length > 0 && filtered.every(m => tempSelected.some(s => s.id === m.id));

  const toggle = (member: Recipient) => {
  ...
  const toggleAll = () => {
    if (isAllSelected) {
      setTempSelected(prev => prev.filter(s => !filtered.some(f => f.id === s.id)));
    } else {
      const toAdd = filtered.filter(f => !tempSelected.some(s => s.id === f.id));
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-md">
      <div className="w-full max-w-lg bg-3 rounded-modal shadow-card-soft overflow-hidden flex flex-col max-h-[80vh]">
        {/* 헤더 */}
        <div className="px-xl py-lg border-b border-border-light flex justify-between items-center bg-3">
          <div>
            <h2 className="text-Heading-2 font-bold text-text-dark-grey">수신자 검색</h2>
            <p className="text-Body-2 text-text-grey-blue mt-xs">선택됨: {tempSelected.length}명</p>
          </div>
        ...
        <div className="p-md border-b border-border-light">
          <div className="relative">
            <input
              className="w-full bg-input-bg-light border border-border-light rounded-button px-md py-sm pl-xl text-Body-2 focus:ring-2 focus:ring-secondary-mint outline-none"
              type="text"
        ...
        {/* 전체 선택 */}
        <div className="px-lg py-sm border-b border-border-light flex items-center justify-between bg-bg-main-light-blue/20">
          <label className="flex items-center gap-sm cursor-pointer text-Body-2 text-text-dark-grey">
            <input
              type="checkbox"
          ...
          <span className="text-Label text-text-grey-blue">{filtered.length}명 표시 중</span>
        </div>

        {/* 목록 */}
        <div className="flex-1 overflow-y-auto divide-y divide-border-light">
          {filtered.map(member => {
            const checked = tempSelected.some(s => s.id === member.id);
            return (
              <label key={member.id} className="flex items-center gap-md px-lg py-md cursor-pointer hover:bg-bg-main-light-blue/20 transition-colors">
                <input
                  type="checkbox"
                  className="w-4 h-4 accent-primary-coral"
                  checked={checked}
                ...
                <div className="flex-1">
                  <p className="text-Body-2 font-medium text-text-dark-grey">{member.name}</p>
                  <p className="text-Label text-text-grey-blue">{member.phone} · {member.group}</p>
                </div>
                {checked && <Check size={16} className="text-primary-coral" />}
              </label>
            );
          })}
          {filtered.length === 0 && (
            <div className="p-xl text-center text-text-grey-blue text-Body-2">검색 결과가 없습니다.</div>
          )}
        </div>
        ...
        <div className="px-xl py-lg border-t border-border-light bg-bg-main-light-blue/5 flex justify-end gap-md">
          <button
            className="px-xl py-md rounded-button border border-border-light text-text-grey-blue hover:bg-3 transition-colors"
            onClick={onClose}
          >
            취소
          </button>
          <button
            className="px-xl py-md rounded-button bg-primary-coral text-white font-semibold hover:opacity-90 transition-opacity"
            onClick={() => onConfirm(tempSelected)}
          >
            선택 완료 ({tempSelected.length}명)
          </button>
        </div>
      </div>
    </div>
...
export default function MessageSend() {
  ...
  // SMS/LMS 분기: 90자 이하 SMS(70원), 초과 LMS(30원 → 실제 비용)
  const effectiveCost = sendForm.channel === 'sms'
    ? (contentLen > 90 ? 30 : 70)
    : channelCfg.costPerMsg;

  const recipientCount = sendForm.useAllMembers
    ? 1240
    : sendForm.recipients.length;

  const totalCost = recipientCount * effectiveCost;

  // SMS 타입 표시
  const smsTypeLabel = sendForm.channel === 'sms'
    ? (contentLen > 90 ? 'LMS' : 'SMS')
    : null;
    ...
    <div className="space-y-lg animate-in fade-in duration-500">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-md">
        <StatCard
          label="보유 포인트"
          value="52,480 P"
          icon={<RefreshCw className="text-secondary-mint" />}
          description="약 SMS 2,624건 발송 가능"
          onClick={() => alert('포인트 충전 페이지로 이동')}
          variant="mint"
        />
          ...
          <div className="flex justify-between items-start">
            <div>
              ...
              <div className="flex items-center gap-sm">
                <select
                  className="bg-input-bg-light border-none rounded-button px-md py-sm text-Body-1 focus:ring-2 focus:ring-secondary-mint"
                  value={sendForm.senderNumber}
                  onChange={(e) => setSendForm({...sendForm, senderNumber: e.target.value})}
                >
      ...
      <div className="grid grid-cols-1 xl:grid-cols-2 gap-lg">
          ...
          <div className="space-y-md">
            ...
            <div>
              <div className="flex justify-between items-center mb-xs">
                <label className="block text-Label text-text-dark-grey font-semibold">
                  수신자 선택 <span className="text-error">*</span>
                </label>
                <span className="text-Label text-text-grey-blue">
              ...
              <div className="flex gap-sm">
                <div className="flex-1 bg-input-bg-light border border-border-light rounded-button p-sm min-h-[44px] flex flex-wrap gap-xs items-center">
                  {sendForm.useAllMembers ? (
                    <span className="bg-bg-soft-peach text-primary-coral px-sm py-[2px] rounded-full text-Label flex items-center gap-xs border border-primary-coral/20">
                      ...
                      전체 회원 (1,240명)
                      <button
                        className="hover:text-error ml-xs"
                        onClick={() => setSendForm(prev => ({ ...prev, useAllMembers: false }))}
                      >
                        <X size={12} />
                      </button>
                    </span>
                  ) : sendForm.recipients.length === 0 ? (
                    <span className="text-text-grey-blue text-Body-2">수신자를 선택하세요</span>
                  ) : (
                    sendForm.recipients.map(r => (
                      <span key={r.id} className="bg-bg-soft-peach text-primary-coral px-sm py-[2px] rounded-full text-Label flex items-center gap-xs border border-primary-coral/20">
                        {r.name}
                        <button className="hover:text-error" onClick={() => removeRecipient(r.id)}>
                          <X size={12} />
                        </button>
                      </span>
                    ))
                ...
                <div className="flex flex-col gap-xs">
                  <button
                    className="bg-bg-soft-mint text-secondary-mint px-md py-sm rounded-button font-bold text-Body-2 hover:bg-secondary-mint hover:text-white transition-colors"
                  ...
                  <button
                    className={cn(
                      "px-md py-sm rounded-button font-bold text-Body-2 transition-colors",
                      sendForm.useAllMembers
                        ? "bg-bg-soft-peach text-primary-coral"
                        : "bg-bg-main-light-blue text-text-grey-blue hover:bg-border-light"
                    )}
                    onClick={() => setSendForm(prev => ({ ...prev, useAllMembers: !prev.useAllMembers, recipients: [] }))}
                  >
                    전체 선택
                  </button>
                </div>
            ...
            {/* 발송 채널 */}
            <div>
              <label className="block text-Label text-text-dark-grey mb-xs font-semibold">
                ...
                {[
                  ...
                  >
                    <input
                      ...
                      onChange={() => setSendForm({...sendForm, channel: item.id})}
                    />
                    {item.icon}
            ...
            {/* 발송 비용 표시 */}
            <div className="bg-bg-main-light-blue/30 rounded-card-normal p-md flex items-center justify-between border border-border-light">
              <div className="flex items-center gap-sm text-text-grey-blue text-Body-2">
                <AlertCircle size={14} className="text-secondary-mint" />
                <span>
                  {sendForm.channel === 'sms'
                    ? `${smsTypeLabel} ${effectiveCost}원/건`
                    : sendForm.channel === 'push'
                      ? '앱 푸시 무료'
                      : `알림톡 ${effectiveCost}원/건`
                  }
                  {' · '}수신자 {recipientCount.toLocaleString()}명
                </span>
              </div>
              <div className="text-right">
                <p className="text-Label text-text-grey-blue">예상 발송 비용</p>
                <p className="text-Body-1 font-bold text-primary-coral">
            ...
            {sendForm.channel === 'sms' && (
              <div>
                ...
                <input
                  className="w-full bg-input-bg-light border border-border-light rounded-button px-md py-sm text-Body-1 focus:ring-2 focus:ring-primary-coral outline-none"
                  type="text"
                  placeholder="LMS 발송 시에만 노출됩니다."
            ...
            <div>
              <div className="flex justify-between items-center mb-xs">
                ...
                <span className={cn(
                  ...
                  isOverLimit ? "text-error" : contentLen > maxLen * 0.9 ? "text-warning" : "text-text-grey-blue"
                )}>
                  {contentLen.toLocaleString()} / {maxLen.toLocaleString()}자
                  {sendForm.channel === 'sms' && (
                    <span className="ml-xs text-[11px]">
                      ({smsTypeLabel})
                    </span>
                  )}
              ...
              <textarea
                className={cn(
                  "w-full bg-input-bg-light border rounded-button px-md py-sm text-Body-1 focus:ring-2 outline-none resize-none",
                  isOverLimit
                    ? "border-error focus:ring-error/30"
                    : "border-border-light focus:ring-primary-coral"
                )}
                rows={6}
                placeholder="내용을 입력하세요. {이름}, {상품명} 등 치환 변수를 사용할 수 있습니다."
                value={sendForm.content}
                onChange={(e) => setSendForm({ ...sendForm, content: e.target.value })}
              />
              {/* 글자 수 시각적 게이지 */}
              <div className="mt-xs h-1 rounded-full bg-border-light overflow-hidden">
                <div
                  className={cn(
                    "h-full rounded-full transition-all",
                    isOverLimit ? "bg-error" : contentLen > maxLen * 0.9 ? "bg-warning" : "bg-secondary-mint"
                  )}
              ...
              <div className="mt-sm flex flex-wrap gap-xs">
                {['{이름}', '{상품명}', '{만료일}', '{잔여횟수}', '{센터명}'].map(tag => (
                  <button
                    className="text-Label bg-3 border border-border-light px-sm py-[2px] rounded-full text-text-grey-blue hover:border-primary-coral hover:text-primary-coral transition-colors"
                    key={tag}
                    onClick={() => setSendForm({ ...sendForm, content: sendForm.content + tag })}
                  >
                    {tag}
                  </button>
                ))}
            ...
            <div className="pt-md border-t border-border-light flex items-center justify-between">
              <div className="flex items-center gap-sm">
                <input
                  ...
                  onChange={(e) => setSendForm({...sendForm, isReserved: e.target.checked})}
                />
                <label className="text-Body-2 text-text-dark-grey cursor-pointer" htmlFor="reserve">예약 발송</label>
                {sendForm.isReserved && (
                  <input
                    className="bg-input-bg-light border border-border-light rounded-button px-sm py-xs text-Body-2 outline-none"
                    type="datetime-local"
                    value={sendForm.reserveDate}
                    onChange={(e) => setSendForm({ ...sendForm, reserveDate: e.target.value })}
                  />
                )}
              </div>
              <button
                className="bg-primary-coral text-white px-xl py-md rounded-button font-bold text-Body-1 shadow-md shadow-primary-coral/20 hover:scale-[1.02] active:scale-[0.98] transition-all flex items-center gap-sm disabled:opacity-50 disabled:cursor-not-allowed disabled:hover:scale-100"
        ...
        <div className="space-y-lg">
          <FormSection
            title="자동 알림 설정"
            description="특정 이벤트 발생 시 자동으로 메시지를 발송합니다."
            columns={1}
            actions={
              <button
              ...
              <div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-sm">
                  {[
                    ...
                    <div
                      className="flex items-center justify-between p-md bg-bg-main-light-blue/30 rounded-card-normal border border-transparent hover:border-border-light transition-all"
                      key={item.key}
                    >
                      <div className="flex items-center gap-sm">
                      ...
                      <div className="flex items-center gap-sm">
                        ...
                        <Toggle
                          ...
                          onChange={() => setAutoAlarms({...autoAlarms, [item.key]: !autoAlarms[item.key as keyof typeof autoAlarms]})}
                        />
                      </div>
                    </div>
                  ))}
      ...
      <div className="space-y-md animate-in fade-in duration-500">
        <div className="flex flex-col md:flex-row gap-md items-end justify-between bg-3 p-lg rounded-card-normal border border-border-light shadow-card-soft">
          <div className="flex-1 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-md w-full">
            <div>
              ...
              <div className="flex items-center gap-xs">
                <input
                  className="flex-1 bg-input-bg-light border border-border-light rounded-button px-sm py-xs text-Body-2 outline-none"
                  type="date"
            ...
            <div>
              ...
              <select
                className="w-full bg-input-bg-light border border-border-light rounded-button px-sm py-xs text-Body-2 outline-none"
            ...
            <div className="lg:col-span-2">
              ...
              <div className="relative">
                <input
                  className="w-full bg-input-bg-light border border-border-light rounded-button px-md py-xs pl-xl text-Body-2 focus:ring-2 focus:ring-secondary-mint outline-none"
                  type="text"
                  placeholder="수신번호 또는 내용 검색"
          ...
          <div className="flex gap-sm w-full md:w-auto">
            <button
              className="flex-1 md:flex-none bg-bg-main-light-blue text-text-dark-grey px-lg py-sm rounded-button font-bold text-Body-2 hover:bg-border-light transition-colors"
              onClick={() => setHistoryFilters({ search: '', dateStart: '', dateEnd: '', type: '' })}
            >
              초기화
            </button>
            <button
              className="flex-1 md:flex-none bg-secondary-mint text-white px-xl py-sm rounded-button font-bold text-Body-2 shadow-md shadow-secondary-mint/20 hover:scale-[1.02] active:scale-[0.98] transition-all"
              onClick={() => alert('조회 필터가 적용되었습니다.')}
            >
              조회하기
            </button>
          </div>
        ...
        <DataTable
          ...
          pagination={{ page: 1, pageSize: 10, total: 124 }}
        />
      </div>
    );
  };
  ...
  return (
    <AppLayout>
      <div className="max-w-[1400px] mx-auto">
        <PageHeader
          title="메시지 발송 (알림톡/SMS)"
          description="회원들에게 다양한 채널을 통해 소식을 전하고 자동 알림을 관리합니다."
        />
