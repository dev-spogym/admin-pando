...
export default function Dashboard() {
  ...
  // Mock 데이터 - 회원 현황
  const memberStats = [
    { id: "UI-010", label: "전체 회원", value: "1,284", icon: <Users />, change: { value: 2.5, label: "전월 대비" }, variant: "default", viewId: 967 },
    { id: "UI-011", label: "활성 회원", value: "945", icon: <UserCheck />, change: { value: 1.2, label: "전체 73.6%" }, variant: "default", viewId: 967 },
    { id: "UI-012", label: "만료 임박", value: "42", icon: <Clock />, change: { value: -0.5, label: "30일 이내" }, variant: "peach", viewId: 967 },
    { id: "UI-013", label: "만료 회원", value: "156", icon: <UserMinus />, change: { value: 0.8, label: "전체 12.1%" }, variant: "default", viewId: 967 },
    { id: "UI-014", label: "예정 회원", value: "28", icon: <Calendar />, change: { value: 4.2, label: "대기 중" }, variant: "default", viewId: 967 },
    { id: "UI-015", label: "미등록 회원", value: "113", icon: <UserPlus />, change: { value: -2.1, label: "이용권 없음" }, variant: "default", viewId: 967 },
  ];
  ...
  const expiringMembers = [
    { id: 1, name: "유재석", expiry: "2024.05.25", dday: "D-5" },
    { id: 2, name: "지석진", expiry: "2024.05.22", dday: "D-2" },
  ];
    ...
    <AppLayout >
      ...
      {showBanner && (
        <div className="mb-lg relative flex items-center justify-between rounded-card-normal bg-6 px-lg py-md border border-0/10 shadow-sm animate-in fade-in slide-in-from-top-4 duration-500" >
          ...
          <div className="flex items-center gap-md" >
            <button 
              className="rounded-button bg-0 px-md py-sm text-Label font-bold text-3 hover:bg-opacity-90 transition-all" onClick={() => moveToPage(977)}>
              기능 보기
            </button>
            <button 
              className="text-5 hover:text-4 transition-colors" onClick={() => setShowBanner(false)}>
              <X size={20}/>
            </button>
          </div>
        </div>
      ...
      <PageHeader title="대시보드" description="스포짐 종각점의 실시간 센터 운영 현황입니다." actions={
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 xl:grid-cols-6 gap-md mb-xl" >
        {memberStats.map((stat) => (
          <StatCard key={stat.id} label={stat.label} value={stat.value} icon={stat.icon} change={stat.change} variant={stat.variant as "default" | "peach" | "mint"} onClick={() => moveToPage(stat.viewId)}/>
        ))}
      </div>
            ...
            <div className="flex items-center gap-xl" >
              <div className="relative h-24 w-24" >
                <svg className="h-full w-full rotate-[-90deg]" viewBox="0 0 36 36">
                  ...
                  <circle 
                    className="stroke-0" cx="18" cy="18" r="16" fill="none" strokeWidth="4" strokeDasharray="55 100"/>
                  <circle 
                    className="stroke-1" cx="18" cy="18" r="16" fill="none" strokeWidth="4" strokeDasharray="45 100" strokeDashoffset="-55"/>
                </svg>
            ...
            <div className="p-sm" >
              <DataTable columns={[
                  ...
                  { 
                    ...
                    header: "D-Day", 
                    align: "center",
                    render: (val) => <StatusBadge variant="warning">{val}</StatusBadge>
                  },
                  {
                    key: "action",
                    header: "",
      ...
      <div className="mt-xxl grid grid-cols-1 lg:grid-cols-2 gap-lg" >
        <div className="group relative overflow-hidden rounded-card-strong bg-gradient-to-r from-[#FF751A] to-[#FFD1B3] p-xl text-3 shadow-lg cursor-pointer transition-transform hover:scale-[1.01]" onClick={() => moveToPage(983)}>
        ...
        <div className="group relative overflow-hidden rounded-card-strong bg-gradient-to-r from-[#121212] to-[#969696] p-xl text-3 shadow-lg cursor-pointer transition-transform hover:scale-[1.01]" onClick={() => moveToPage(980)}>
          <div className="relative z-10" >
